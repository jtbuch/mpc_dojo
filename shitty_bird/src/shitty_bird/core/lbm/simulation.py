...
class Simulation:
    ...
    def __copy__(self):
        new_sim = self.__class__.__new__(self.__class__)
        # Copy the instance dictionary without triggering __getattr__
        new_sim.__dict__ = self.__dict__.copy()
        return new_sim

    def copy_with_fluid_state(self, new_fluid_state):
        new_sim = copy.copy(self)
        new_sim.fluid_state = new_fluid_state
        return new_sim
